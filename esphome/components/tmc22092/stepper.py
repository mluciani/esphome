...
TMC22092 = tmc_ns.class_("TMC22092", stepper.Stepper, cg.Component)
TMC22092SetupAction = tmc_ns.class_("TMC22092SetupAction", automation.Action)
...
CONFIG_SCHEMA = cv.All(
    cv.Schema({
        cv.Required(CONF_STEPPERS): cv.All(cv.ensure_list(cv.Schema({
            cv.GenerateID(): cv.declare_id(TMC22092),
...
@automation.register_action(
    ...
    cv.Schema(
        {
            cv.Required(CONF_ID): cv.use_id(TMC22092),
            cv.Required(CONF_NAME): cv.string,
...
def tmc22092_setup_to_code(config, action_id, template_arg, args):
