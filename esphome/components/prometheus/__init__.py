...
CUSTOMIZED_ENTITY = cv.Schema(
    {cv.Optional(CONF_ID): cv.string_strict, cv.Optional(CONF_NAME): cv.string_strict},
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_CUSTOMIZE, default={}): cv.Schema(
            {
...
async def to_code(config):
    ...
    for key, value in config[CONF_CUSTOMIZE].items():
        entity = await cg.get_variable(key)
        if CONF_ID in value:
            cg.add(var.add_custom_id(entity, value[CONF_ID]))
        if CONF_NAME in value:
            cg.add(var.add_custom_name(entity, value[CONF_NAME]))
